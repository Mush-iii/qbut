    ...
    if (app->desktopIntegration()->isActive())
    {
        ...
        else if (initialState == WindowState::Minimized)
        {
            ...
            if (pref->minimizeToTray())
            {
                ...
                {
                    app->desktopIntegration()->showNotification(tr("qButtorrent is minimized to tray"), tr("This behavior can be changed in the settings. You won't be reminded again."));
...
void MainWindow::setTitleSuffix(const QString &suffix)
{
    ...
    m_windowTitle = QStringLiteral("qButtorrent " QBT_VERSION)
...
void MainWindow::closeEvent(QCloseEvent *e)
{
    ...
    if (!m_forceExit && app()->desktopIntegration()->isActive() && goToSystrayOnExit && !this->isHidden())
    {
        ...
        {
            app()->desktopIntegration()->showNotification(tr("qButtorrent is closed to tray"), tr("This behavior can be changed in the settings. You won't be reminded again."));
    ...
    if (pref->confirmOnExit() && hasActiveTorrents)
    {
        if (e->spontaneous() || m_forceExit)
        {
            if (!isVisible())
                show();
            QMessageBox confirmBox(QMessageBox::Question, tr("Exiting qButtorrent"),
                                   // Split it because the last sentence is used in the WebUI
                                   tr("Some files are currently transferring.") + u'\n' + tr("Are you sure you want to quit qButtorrent?"),
