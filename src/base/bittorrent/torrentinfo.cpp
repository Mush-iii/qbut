...
namespace
{
    ...
    // libtorrent 2.1 deprecated `orig_files()`, `layout()` returns the original file storage
    const lt::file_storage &nativeOrigFiles(const std::shared_ptr<const lt::torrent_info> &nativeInfo)
    {
        return nativeInfo->layout();
    }
#else
    const lt::file_storage &nativeOrigFiles(const std::shared_ptr<const lt::torrent_info> &nativeInfo)
    {
        return nativeInfo->orig_files();
    }
#endif
}
    ...
    : m_nativeInfo {std::make_shared<const lt::torrent_info>(nativeInfo)}
{
    ...
    const lt::file_storage &fileStorage = nativeOrigFiles(m_nativeInfo);
...
QString TorrentInfo::name() const
{
    ...
    return QString::fromStdString(nativeOrigFiles(m_nativeInfo).name());
}
...
Path TorrentInfo::filePath(const int index) const
{
    ...
    return Path(nativeOrigFiles(m_nativeInfo).file_path(m_nativeIndexes[index]));
}
...
qlonglong TorrentInfo::fileSize(const int index) const
{
    ...
    return nativeOrigFiles(m_nativeInfo).file_size(m_nativeIndexes[index]);
}
...
qlonglong TorrentInfo::fileOffset(const int index) const
{
    ...
    return nativeOrigFiles(m_nativeInfo).file_offset(m_nativeIndexes[index]);
}
...
TorrentInfo::PieceRange TorrentInfo::filePieces(const int fileIndex) const
{
    ...
    const lt::file_storage &files = nativeOrigFiles(m_nativeInfo);
