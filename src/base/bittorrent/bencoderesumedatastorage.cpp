...
BitTorrent::LoadResumeDataResult BitTorrent::BencodeResumeDataStorage::loadTorrentResumeData(const QByteArray &data, const QByteArray &metadata) const
{
    ...
    if (!metadata.isEmpty())
    {
        ...
        try
        {
            torrentInfo = std::make_shared<lt::torrent_info>(torrentInfoRoot);
        }
        catch (const lt::system_error &err)
        {
            return nonstd::make_unexpected(tr("Cannot parse torrent info: %1").arg(QString::fromStdString(err.code().message())));
        }
