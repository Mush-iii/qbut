...
void TorrentCreator::run()
{
    ...
    try
    {
        ...
        {
#if LIBTORRENT_VERSION_NUM >= 20100
            // libtorrent 2.1 removed the deprecated add_files()/file_storage based API
        ...
        else
        {
            ...
            {
#if LIBTORRENT_VERSION_NUM >= 20100
                fs.emplace_back(fileName.toStdString(), fileSizeMap[fileName]);
#else
...
#if LIBTORRENT_VERSION_NUM >= 20100
        lt::create_torrent newTorrent {std::move(fs), m_params.pieceSize, toNativeTorrentFormatFlag(m_params.torrentFormat)};
...
#if LIBTORRENT_VERSION_NUM >= 20100
    // libtorrent 2.1 removed the deprecated add_files()/file_storage based API
    return lt::create_torrent {lt::list_files(inputPath.toString().toStdString(), fileFilter)
        , pieceSize, toNativeTorrentFormatFlag(torrentFormat)}.num_pieces();
#elif defined(QBT_USES_LIBTORRENT2)
    lt::file_storage fs;
