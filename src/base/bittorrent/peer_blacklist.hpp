...
lt::tcp::endpoint get_peer_ip(const lt::peer_info& info)
{
  return info.remote_endpoint();
}
#else
lt::tcp::endpoint get_peer_ip(const lt::peer_info& info)
{
...
bool is_bad_peer(const lt::peer_info& info)
{
  ...
  QString country = Net::GeoIPManager::instance()->lookup(QHostAddress(get_peer_ip(info).data()));
...
bool is_offline_downloader(const lt::peer_info& info)
{
  ...
  unsigned short port = get_peer_ip(info).port();
  QString country = Net::GeoIPManager::instance()->lookup(QHostAddress(get_peer_ip(info).data()));
